...
export function generateCartoonBeadPattern(image,options={}){
 ...
 // No forced tier budget. Explicit limits are ceilings, never color targets.
 const maxColors=options.maxColors??null;
 const paletteSize=new Set((options.palette||[]).map(c=>c.code)).size;
 if(!paletteSize)throw Error('Real bead palette required');
 if(maxColors!==null&&(!Number.isSafeInteger(maxColors)||maxColors<1||maxColors>paletteSize))throw Error(`最大色数须为 1～${paletteSize} 的整数`);
 ...
 const mergedColors=maxColors===null?0:applyColorBudget(entries,maxColors,delta);
