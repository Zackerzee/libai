...
export function generateCartoonBeadPattern(image,options={}){
 ...
 // Stroke regions (thin lines: hair highlights, goggle glints, whiskers) have
 // small area relative to their boundary length. A stroke covering ~25% of a
 // cell already deserves the bead -- otherwise thin strokes fragment into
 // dashed dots under area-majority voting. Compact blobs and fills never
 // qualify, and the stroke tone must clearly differ from the area winner.
 ...
 for(let y=0;y<size.height;y++)for(let x=0;x<size.width;x++){
  ...
  if(strokes.size){
   let strokeId=-1,strokeN=0;
   for(const [id,n] of counts)if(strokes.has(id)&&n>strokeN){strokeId=id;strokeN=n;}
   if(strokeId>=0&&strokeN>=samples.length*.25){const sc=seg.regions[strokeId].color;if(delta(sc,rgb)>=15)rgb=sc;}
  }
