...
const INSTRUCTIONAL_REVIEW_WORDS = [
  "参考点",
  "参考价值",
  "参考意义",
  "对其他顾客",
...
function looksTooFormal(text) {
  ...
  const formalPatterns = [
    ...
    /非常适合.{0,20}人群/,
    /让人感受到/,
    /主要看.{0,18}(图案|颜色|耗时|材料|环境)/,
    /适合给.{0,12}参考/,
    /给.{0,12}一个.{0,8}参考/,
    /拍.{0,12}(过程图|成品图).{0,16}(参考|直观|判断)/,
  ];
...
function getInvalidReason(review, projectName, keywords = "", { checkSimilarity = true } = {}) {
  ...
  if (hasAnyWord(text, INSTRUCTIONAL_REVIEW_WORDS)) return "instructional";
...
你的任务是：根据顾客选择的手作项目、平台、语气和真实关键词，生成一条像顾客自己写的正向中文评价。

[高质量评价标准]
1. 必须是正向评价，但要像真实顾客写的，不要像商家广告。
2. 内容要让没来过的人能看懂真实体验，可以自然提到 2 到 3 类信息：体验项目、适合人群/消费场景、上手难度或耗时感受、材料/颜色/图案选择、成品是否能带走、位置是否方便。
...
18. 禁止写成给顾客的攻略或提示，不要出现“参考点、参考价值、更直观、别人看评价、后面想来的人、建议拍、现场拍、过程图和成品图、需要交给店员”等说明式表达。

[真实体验写法]
1. 拼豆评价可以写这些真实体验点：色号和豆子是否好找、图案选择、工具/桌面是否清楚、小图是否适合新手、大图是否需要留够时间、成品由店员帮忙熨烫处理、是否适合亲子或朋友一起坐下来做。
...
8. 如果关键词提到垫板、镊子、清洁刷、压豆、控温、补救、换板，可以写工具齐全或店员处理细节；否则只写“工具比较清楚/成品由店员处理”。
...
function buildUserPrompt(context) {
  ...
  return [
    ...
    `评价语气：【${context.tone}】。`,
    keywordsText,
    "请写一条高质量正向评价：语气像顾客自己写的，可以自然提到适合谁、项目难不难、耗不耗时间、材料选择、作品效果、是否能带走等。",
    "如果是拼豆，可以围绕色号/豆子/图案/工具/图纸大小/熨烫处理/适合新手或亲子这些体验点展开，每次只选其中 2 到 4 个。",
    ...
    "不要写成攻略或说明，不要出现“参考点、参考价值、更直观、别人看评价、后面想来的人、建议拍、现场拍、过程图和成品图、需要交给店员”等话。",
...
function makeReferenceFallback(context) {
  ...
  if (project === "拼豆") {
    const options = [
      `这次做拼豆还蛮顺的，颜色和图案选择比较多，前面选款会花一点时间。制作过程不算复杂，但要耐心慢慢摆，拼完店员会帮忙熨好，最后能带走自己的小作品，店里节奏也不赶。`,
      `拼豆适合想坐下来慢慢做的人，选图案和找颜色会占一点时间，小图上手比较快。照着图纸一步步摆，做着做着会比较专注，最后成品由店员处理好，拿到手时还挺开心。`,
      hasChild
        ? "带孩子来做拼豆比想象中省心，孩子自己选图案，大人在旁边帮忙找颜色。过程需要一点耐心，但不会太难，做完后店员会把成品处理好，孩子拿着自己的小作品还蛮开心。"
        : "第一次做拼豆，我选了小一点的图，颜色和图案都比较清楚，照着图纸慢慢摆就行。过程需要一点耐心，但不算难，拼好后店员帮忙处理成品，最后带走留念也不错，整体还蛮轻松。",
      hasLargeWork
        ? "如果选大一点的拼豆图，时间确实要留充足一些，选色和对图纸都会更细。慢慢把图案摆出来的过程很适合静下来，最后店员帮忙处理成品，拿到手会比只看图片更有感觉。"
        : "这次选了一个不算太大的拼豆图，刚开始找颜色有点慢，后面就顺多了。成品处理好之后比想象中可爱，拿走的时候还特意多看了几眼。店里节奏不赶，适合安静坐下来做一会儿。",
    ];
  ...
  const options = [
    `${audience}做${project}比较合适，项目上手不算复杂，材料和样式选择比较清楚，可以按照自己的节奏慢慢完成。做完以后有作品可以带走，第一次尝试手作也不会太紧张。`,
    `${project}适合想找个室内活动的人，前面主要是选材料和样式，后面按步骤慢慢做。整体难度不会一下子太高，完成后能看到自己的作品，拍几张照片留念也挺合适。`,
    hasFriend
      ? `朋友一起做${project}会比较合适，过程可以边做边聊，不需要一直赶进度。材料选择比较清楚，步骤也容易跟上，最后有作品可以带走，比只坐着聊天更有参与感。`
      : `${project}比较适合想安静坐下来做点东西的人，材料和工具都比较清楚，按照步骤完成就行。成品能带走，做完看着自己的作品，会觉得这段时间花得挺值。`,
  ];
  return options[Math.floor(Math.random() * options.length)];
}
